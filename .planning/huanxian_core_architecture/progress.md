...
- Fixed the "standing still attack stops working" feel regression: light attack no longer spends Sanity, and `CombatResourceController` now regenerates Sanity after a short delay so defensive actions and heavy attacks cannot silently exhaust all basic combat input.
